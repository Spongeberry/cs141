...
/**
  ...
  */
  ...
  T operator * (const Vector & v) const {
    T sum = 0;
    int minSize = min(static_cast<int>(sz), static_cast<int>(v.sz));
  ...
  */
  ...
  Vector operator + (const Vector & v) const {
    int bigger_size = max(static_cast<int>(sz), static_cast<int>(v.sz));
    Vector Vec(bigger_size);
    for (int i = 0; i < bigger_size; ++i){
        Vec.buf[i] = (i < this->size() ? buf[i] : 0) + (i < v.size() ? v.buf[i] : 0);
    }
    return Vec;
  }
